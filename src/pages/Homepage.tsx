...
export function Homepage() {
	...
	// Effect to sync token with localStorage changes
	useEffect(() => {
	...
	const saveDownloadToDB = async (item: any, token: string) => {
		try {
			const response = await fetch(`${API_BASE_URL}/download-history`, {
				method: 'POST',
				headers: {
			...
			if (!response.ok) {
				console.error('Failed to save download history:', response.statusText);
			} else {
	...
	const downloadYouTubeVideo = async (url: string, quality: number) => {
		const apiUrl = `${API_BASE_URL}/download?platform=youtube&url=${encodeURIComponent(
			url
		)}&quality=${quality}`;
		...
		if (token) {
			...
			await saveDownloadToDB(downloadItem, token);
		}
	...
	const downloadFacebookVideo = async (url: string) => {
		const apiUrl = `${API_BASE_URL}/download?platform=facebook&url=${encodeURIComponent(
			url
		)}`;
	...
	const downloadInstagramMedia = async (url: string) => {
		const apiUrl = `${API_BASE_URL}/download?platform=instagram&url=${encodeURIComponent(
			url
		)}`;
	...
	return (
		<div className='min-h-screen grid gap-10'>
			...
			<section
				id='home'
				className='text-center py-15 flex flex-col items-center justify-center gap-5'
			>
				{/* DOWNLOADER */}
			...
			{/* GUIDE SECTION */}
			<section id='guide' className='flex items-center justify-center'>
				<GuideContent />
			</section>
			...
			{/* REVIEWS SECTION */}
			<section id='reviews' className='flex items-center justify-center'>
				<ReviewContent />
			</section>
		</div>
	);
